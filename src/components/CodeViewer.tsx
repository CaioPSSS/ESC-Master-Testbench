...
export function CodeViewer() {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="mb-4 flex justify-between items-start cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
        <div>
          <h2 className="text-sm font-bold text-white mb-1 uppercase tracking-tight flex items-center gap-2">
            Código dos Microcontroladores (LoRa)
        ...
        {isExpanded && (
          <button
            ...
            className="shrink-0 flex items-center gap-1.5 text-[10px] px-2 py-1 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 transition-colors border border-slate-600 font-bold uppercase tracking-wider ml-4"
          >
      ...
      {isExpanded && (
        <>
          ...
          <div className="flex mb-3 gap-1">
            <button
              onClick={(e) => { e.stopPropagation(); setActiveTab('esp32'); setCopied(false); }}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-t-md text-[11px] font-bold uppercase tracking-wider transition-colors border border-b-0 ${
                activeTab === 'esp32'
            ...
            <button
              onClick={(e) => { e.stopPropagation(); setActiveTab('arduino'); setCopied(false); }}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-t-md text-[11px] font-bold uppercase tracking-wider transition-colors border border-b-0 ${
                activeTab === 'arduino'
          ...
          {/* Code Snippet Visual */}
          <div className="bg-[#011627] rounded-b-lg rounded-tr-lg p-5 font-mono text-[11px] leading-relaxed border border-slate-700 shadow-2xl overflow-y-auto flex-1 mb-4">
